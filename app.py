...
CONTENT_MATCH_RULES = {
    ...
    'responsible':    ['负责人', 'responsible', 'verantwortlich', '责任', '检验人', '执行人', '部门', '责任部门'],
}
...
def parse_control_plan(filepath, user_ws_col=None):
    ...
    ws_keywords = [
        ...
        '岗位', 'platz', 'station nr', 'station no', 'op-nr', 'op nr',
    ]
...
.ws-search:focus { outline: none; border-color: var(--primary); }
...
function buildMappingUI(){
    ...
    $$('.map-select').forEach(sel=>{
        const type=sel.dataset.type,field=sel.dataset.field;
        const curMap=type==='header'?STATE.headerMapping:STATE.contentMapping;
        sel.value=curMap[field]||'';
        sel.addEventListener('change',()=>{
            if(type==='header'){STATE.headerMapping[field]=sel.value?parseInt(sel.value):null;}
            else{STATE.contentMapping[field]=sel.value?parseInt(sel.value):null;}
            const badge=sel.parentElement.querySelector('.match-badge');
            if(badge && !badge.classList.contains('match-auto')){
                badge.className='match-badge '+(sel.value?'match-manual':'match-none');
                badge.textContent=sel.value?'✏️':'—';
            }
            updatePreview();
        });
    });
